...
class Accountstatement_model extends CI_Model
{
    ...
    public function canViewAny($user){
        ...
        if ($role)
            return
                ...
                ? auth()->allow() : (in_array('view', explode(',', $role->permission->account_statements))?auth()->allow()
    ...
    public function canCreate($user){
        ...
        if ($role)
            return
                $role->permission->is_admin === '1'
                ? auth()->allow() : (in_array('create', explode(',', $role->permission->account_statements))?auth()->allow()
    ...
    public function canUpdate($user, $model){
        ...
        if ($role)
            return
                $role->permission->is_admin === '1'
                ? auth()->allow() : (in_array('update', explode(',', $role->permission->account_statements))?auth()->allow()
    ...
    public function canDelete($user, $model){
        ...
        if ($role)
            return
                $role->permission->is_admin === '1'
                ? auth()->allow() : (in_array('delete', explode(',', $role->permission->account_statements))?auth()->allow()
